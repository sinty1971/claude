...
# メイン処理: @manifest.yaml を @koji.yaml にリネーム
...
def process_rename(manifest_path)
  ...
  koji_path = File.join(dir_path, '@koji.yaml')

  # すでに @koji.yaml が存在する場合の処理
  if File.exist?(koji_path)
    puts "スキップ: #{manifest_path} (@koji.yaml が既に存在)"
    return
  end

  # ファイルをリネーム
  FileUtils.mv(manifest_path, koji_path)
  
  puts "リネーム: #{manifest_path} → #{koji_path}"
end

# エントリーポイント
if __FILE__ == $PROGRAM_NAME
  if ARGV.length < 1
    puts '使用方法: ruby rename_manifest_to_koji.rb "C:/path/to/2 工事"'
    puts ''
    puts '例:'
    puts '  ruby scripts/rename_manifest_to_koji.rb "C:/Users/user/penguin/2 工事"'
    puts '  ruby scripts/rename_manifest_to_koji.rb ~/penguin/2\ 工事'
    exit 1
  end
